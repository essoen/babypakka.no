...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${pkg.name} – Produkter | Babypakka.no`,
      description: pkg.description || `Se produktene i ${pkg.name} fra Babypakka.no`,
      alternates: {
        canonical: `https://babypakka.no/pakker/${id}`,
      },
      openGraph: {
        title: `${pkg.name} | Babypakka.no`,
        description: pkg.description || `Se produktene i ${pkg.name}`,
        type: 'website',
        locale: 'nb_NO',
      },
    };
  } catch {
    return { title: 'Produkter | Babypakka.no' };
  }
}
    ...
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-8">
        ...
        <p className="mt-1 text-sm text-baby-text-light">
          Fast pris. Velg fritt blant produktene nedenfor i onboarding.
        </p>
      </div>
      ...
      {pkg.products && pkg.products.length > 0 ? (
        <section className="mt-12">
          <h2 className="text-2xl font-bold text-baby-text">Tilgjengelige produkter</h2>
          <p className="mt-2 text-baby-text-light">
            Velg de produktene du vil ha når du starter abonnementet.
          </p>
        ...
        <section className="mt-12">
          ...
          <p className="mt-4 text-baby-text-light">
            Produktoversikten for denne fasen er ikke tilgjengelig ennå.
          </p>
        </section>
      )}
